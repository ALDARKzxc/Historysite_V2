import { Flame, Trophy, Languages, Zap, Swords, CheckCircle } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { useLanguage } from '@/LanguageContext';
import { popIn, staggerGrid, fadeUp, cardHover, inView, spring } from '@/lib/animations';
...
const FEATURES = [
  ...
  { icon: <Swords className="w-6 h-6 text-purple-500" />, titleKey: 'feat_langlab_title', descKey: 'feat_langlab_desc', color: '#8B5CF6', bg: '#F5F3FF' },
  ...
  return (
    <section className="py-20 bg-[#F5F7FA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <motion.div className="text-center mb-14" variants={fadeUp} initial="hidden" whileInView="show" viewport={inView}>
        ...
        >
          {FEATURES.map((feature, i) => (
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 border border-[#EEF1F7] shadow-sm hover:shadow-xl transition-shadow"
            >
              <motion.div
        ...
        <div className="bg-white rounded-2xl border border-[#EEF1F7] shadow-sm p-8">
          ...
          >
            {LEVELS.map((level, i) => (
              <motion.div key={i} variants={popIn} whileHover={{ y: -6 }} transition={spring} className="flex flex-col items-center gap-2">
        ...
        <div className="text-center mt-12">
          <motion.button
            ...
            whileTap={{ scale: 0.96 }}
            transition={spring}
            className="inline-flex items-center gap-2 px-8 py-4 bg-[#C94B4B] text-white rounded-2xl font-medium font-ui hover:bg-[#b03d3d] transition-colors shadow-xl shadow-[#C94B4B]/25 text-base"
          >
